...
- [x] **Task 1.1:** Viết API cho phép học viên đăng ký vào một lớp (`POST /api/v1/enrollments`). Đảm bảo kiểm tra trùng lặp dữ liệu (Một người không đăng ký 1 lớp 2 lần).
- [x] **Task 1.2:** Viết API hủy đăng ký (Rút học phần) cho phép thay đổi trạng thái.
- [x] **Task 1.3:** Viết API lấy danh sách học viên của một lớp cụ thể (để cung cấp cho giáo viên xem).

## 2. API Điểm danh (Attendances)
- [x] **Task 2.1:** Viết API điểm danh hàng loạt cho một lớp trong một ngày (`POST /api/v1/attendances`).
- [x] **Task 2.2:** Viết logic tính toán **Tỷ lệ % chuyên cần** của một học viên trong một lớp (`GET /api/v1/attendances/summary/{studentId}`).
...
- [x] **Task 3.1:** Viết API nhập điểm (giữa kỳ, cuối kỳ) cho học viên (`POST /api/v1/grades`).
- [x] **Task 3.2:** Viết logic tính Điểm trung bình môn (GPA) tự động trả về khi tra cứu.
